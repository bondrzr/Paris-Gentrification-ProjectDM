---
title: "Effect on BPE"
author: "Pol + Matei"
format: html
---

#Loadinf all of our data + required packages
```{r}
library(dplyr)
library(here)
library(janitor)
library(rvest)
library(stringi)
BPE_Paris_2016 <- readRDS(here("DATA_Processed" , "BPE_Paris_2016_iris.rds"))
BPE_Paris_2018 <- readRDS(here("DATA_Processed" , "BPE_Paris_2018_iris.rds"))
BPE_Paris_2024 <- readRDS(here("DATA_Processed" , "BPE_Paris_2024_iris.rds"))
Gentri_score <- readRDS(here("DATA_Processed" , "gentrif_indices.rds"))
```

#Making sure that they're the same type 
```{r}
BPE_Paris_2016 <- BPE_Paris_2016 %>%
  mutate(dep = as.character(dep))

BPE_Paris_2018 <- BPE_Paris_2018 %>%
  mutate(dep = as.character(dep))

BPE_Paris_2024 <- BPE_Paris_2024 %>%
  mutate(dep = as.character(DEP))
```

#We're adding the names corresponding to the codes to the 24 base 
```{r}
nomenclature_2024 <- read_html(here("DATA_Raw", "BPE24_liste_hierarchisee_TYPEQU.html"))
   
tables <- html_table(nomenclature_2024, fill = TRUE)

nomenclature_bpe <- tables[[1]] %>%
  clean_names() %>%
  rename(code = code,
         libelle = libelle,
         description = description
         ) %>%
  filter(!is.na(code), code != "")

BPE_Paris_2024 <- BPE_Paris_2024 %>%
  left_join(
    nomenclature_bpe %>% select(code, libelle),
    by = c("TYPEQU" = "code")
  )
```

#Reducing bases into a more compact format and harmonising names at the same time 
```{r}
BPE_Paris_2016_red <- BPE_Paris_2016 %>%
  transmute(
    iris = iris_2020,
    year = 2016,
    code = typequ,
    type = Équipement,
    nb = nb_equip,
    geometry
  )

BPE_Paris_2018_red <- BPE_Paris_2018 %>%
  transmute(
    iris = CODE_IRIS,
    year = 2018,
    code = code_type_d_equipements,
    type = nom_type_d_equipements,
    nb = nombre_d_equipements,
    geometry
  )

BPE_Paris_2024_red <- BPE_Paris_2024 %>%
  transmute(
    iris = iris_2020,
    year = 2024,
    code = TYPEQU,
    type = libelle,
    nb = NBEQUIDENT,   
    geometry
  )

BPE_Paris_2024_red <- BPE_Paris_2024_red %>%
  mutate(nb = if_else(is.na(nb), 1L, nb))


```

After checking BPE_2018_red we notice one code missing it's equipement type code A506 after checking in the following file:
<https://www.google.com/url?sa=t&source=web&rct=j&opi=89978449&url=https://www.insee.fr/fr/metadonnees/source/fichier/BPE1621_nb_equip_UU2020_DictionnaireVariables.pdf&ved=2ahUKEwiDz5v9s_yRAxW0dqQEHVQKAMQQFnoECBoQAQ&usg=AOvVaw3Hhj0EDBeKS9FA4Cg8yyg->

A506 = PRESSING-LAVERIE AUTOMATIQUE ( this code is kept the same in 2024)

```{r}
library(dplyr)

BPE_Paris_2018_red <- BPE_Paris_2018_red %>%
  mutate(
    type = if_else(code == "A506", 
                   "PRESSING-LAVERIE AUTOMATIQUE", 
                   type)
  )
```



#Merging BPE into one base + making sure everything is written the same way 
```{r}
BPE_final <- bind_rows(
  BPE_Paris_2016_red %>% mutate(year =2016),
  BPE_Paris_2018_red %>% mutate(year = 2018),
  BPE_Paris_2024_red %>% mutate(year = 2024),
)

BPE_final <- BPE_final %>%
  mutate(
    type = type %>%
      tolower() %>%
      stringi::stri_trans_general("Latin-ASCII")
  )

```

Our analysis limits itself to the effect on the surround businesses ( Tissu commercial) this means we don't need to keep all the data here but the only that concerns Businesses:
SO we'll only keep codes beginning with 
-A5	"AUTRES SERVICES"	Ce sont des commerces de services de proximité dans différentes branches : coiffure, esthétique, pressing, restauration, intérim, vétérinaire 
-B "COMMERCES"	Ce domaine est divisé en trois : les grandes surfaces, les commerces alimentaires et des commerces spécialisés non-alimentaires (qui ne sont pas présents dans le domaine A).

```{r}
library(dplyr)

BPE_final_filtered <- BPE_final %>%
  filter(grepl("^B|^A5", code))

```



```{r}
library(dplyr)

code_uniques <- BPE_final_filtered %>%
  st_drop_geometry() %>%
  group_by(type) %>%
  summarise(n_codes = n_distinct(code),
            codes_list = paste(unique(code), collapse = ", ")
            ) %>% arrange(type)

write.csv2(code_uniques, here("DATA_Processed", "code_uniques.csv"), 
row.names = FALSE, fileEncoding = "UTF-8")

```

#Merging names when small differences across different databases
```{r}
library(dplyr)
library(stringr)

BPE_final_clean <- BPE_final_filtered %>%
  mutate(
    type_clean = case_when(
      str_detect(type, regex("boulangerie", ignore_case = TRUE)) ~ "boulangerie-patisserie",
      str_detect(type, regex("fleuriste", ignore_case = TRUE)) ~ "fleuriste-jardinerie-animalerie",
      str_detect(type, regex("horlogerie", ignore_case = TRUE)) ~ "horlogerie-bijouterie",
      str_detect(type, regex("hypermarche", ignore_case = TRUE)) ~ "hypermarche et grand magasin",
      str_detect(type, regex("institut de beaute|soins de beaute", ignore_case = TRUE)) ~ "institut de beaute-onglerie",
      str_detect(type, regex("librairie|papeterie", ignore_case = TRUE)) ~ "librairie-papeterie-journaux",
      str_detect(type, regex("parfumerie|cosmetique", ignore_case = TRUE)) ~ "parfumerie-cosmetique",
      str_detect(type, regex("restaurant", ignore_case = TRUE)) ~ "restaurant-restauration-rapide",
      str_detect(type, regex("service", ignore_case = TRUE)) ~ "station-service",
      TRUE ~ type
    )
  )

saveRDS(BPE_final_clean, here( "DATA_Processed" , "BPE_final_clean.rds" ))

```

#useful to check for different 
```{r}
code_uniques_clean <- BPE_final_clean %>%
  st_drop_geometry() %>%
  group_by(type_clean) %>%
  summarise(
    n_codes = n_distinct(code),
    codes_list = paste(unique(code), collapse = ", ")
  ) %>%
  arrange(type_clean)

```

