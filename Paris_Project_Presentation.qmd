...
note for BPE: Currently our database for BPE is too large to commit to Github thus it is not available in our data_git folder but a Paris reduced version is.

6) Activity of residents 
<https://www.insee.fr/fr/statistiques/5650708> (2018)
<https://www.insee.fr/fr/statistiques/6543289> (2019)
<https://www.insee.fr/fr/statistiques/7704089> (2020)
<https://www.insee.fr/fr/statistiques/6543289> (2021)
...
file <- here("DATA_Processed", "filo_final.rds")
...
file <- here("DATA_Processed", "JOB_all.rds")
...
file <- here("DATA_Processed", "BPE_Paris_2024_iris.rds")
...
file <- here("DATA_Processed", "BPE_Paris_2018_iris.rds")
...
file <- here("DATA_Processed", "BPE_Paris_2016_iris.rds")
...
Prix_M2 -> this is the price per square meter we created this from our DVF file by dividing Price of "Valeur foncière" by the surface of the place. 
...
pop1564 -> Number of people aged 15 to 64

act1564 -> Number of people either working or unemployed wanting to work aged 15 to 64

actocc1564 -> Number of people active and employed aged 15-64

chom1564 -> Number of unemployed people aged 15-64

sal15p -> Number of people who work for a salary aged 15 and older

actocc15p -> Number of people active and employed aged 15 and older

etud1564 -> Number of students aged 15 to 64

cadres_1564 -> Number of managers aged 15 to 64

prof_inter_1564 -> Number of intellectual workers aged 15-64
...
For FILOSOFI it was quite simple the objective was solely to keep the department that interest us for our analysis and renaming columns for the names to be the same across years.
...
After that we joined all the data into one big table for further analysis later on.
...
The first one was the easiest as it had longitude and latitude, the first part was always to keep the Paris area in our analysis excluding the rest and keeping the columns that we might use in our analysis. Then we tried to get rid of "outliers" this could be when there is unavailable data or some that doesn't make any sense. The biggest part was going from long-lat to IRIS point for this we used a Iris shapefile of france in 2020 because it matched for most of our databases ( our data had to be modified into lambert 93 to match).

We haven't been able to properly finish with older data-base as it doesnt have any long-Latt info so we'll have to do some parcel reattachment which might be a little harder. 

For BPE it was quite similar to the DVF, even tough we had IRIS info they were from years different from 2020 but thankfully we had the geographical location in coordinates which allowed us to spatially reattach the businesses to the 2020iris points. We also filtered to fit our geographical are ( IFF-Paris) 
