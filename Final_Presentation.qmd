...
Although our analysis is succesful in identifying neighbourhoods that have undergone gentrification it is not strongly reflected in the BPE. 
...
Overall when it comes to the BPE it can be for multiple reasons that we do not observe changes. First, commerical change often happens more slowly than residential change. People can move into a neighbourhood relatively quickly as housing prices and income levels change but shops and services are generally much more stable. Many businesses are under long contracts and face high relocation costs. As a result even if the population or income structure changes, the types of businesses present may remain the same for several years. 

As well as this using business categories does not neccesarily make for a viable analysis, it is difficult to differ within categories, for example, a small local cafe may be replaced by a more expensive or trendy cafe but both are counted in the same category.
