...
## d Gentrification score building
...
## f. Effect of gentrification on the “tissu commercial” (BPE results)

We now investigate whether neighbourhood gentrification is associated with a different commercial structure. We use the BPE panel (2016, 2018, 2024), classify establishments into broad business categories, and compare the distribution of these categories across quartiles of the gentrification score.
...
# Assign gentrification quartiles (based on IRIS score)
...
The following figure compares the commercial composition across gentrification quartiles and years. Visually, category shares appear relatively similar across quartiles and fairly stable over time, suggesting that commercial structure may adjust slowly to neighbourhood upgrading.We also see the asbence of bar in 2016 this is due to the fact that no business was part of a 4th quartel neighborhood in the 2016 base.
```{r}
#| echo: false
#| message: false
#| warning: false
...
facet_wrap(
~ score_quartile, nrow = 1,
labeller = labeller(score_quartile = c(
"1" = "Q1 Low gentrification",
"2" = "Quartile 2",
"3" = "Quartile 3",
...
fill = "Category",
title = "Commercial composition by gentrification quartile and year"
) +
theme_minimal() +
...
We also focus on restaurants, a category often linked to gentrification. The evolution of restaurant shares shows only mild differences across quartiles but we are able to say that the shares of restaurants is stronger in the 4th quartile even if it's only by a very small margin.
...
ggplot(df_rest_quartiles,
aes(x = year, y = part, group = score_quartile, color = factor(score_quartile))) +
geom_line(linewidth = 1.1) +
geom_point(size = 2.6) +
scale_color_brewer(palette = "Set1", name = "Gentrification quartile") +
labs(
title = "Restaurant share by gentrification quartile",
x = "Year",
y = "Restaurant share"
) +
theme_minimal() +
...
To answer our question on how gentrification impacts the commercial composition of it's neighborhood, it's important to do some statistical tests. 

We test whether business categories are independent from gentrification quartiles using a Pearson chi-squared test on the contingency table (quartile × category). We also report Cramér’s V as an effect size measure.
...
pivot_wider(names_from = categorie, values_from = nb, values_fill = 0)

chi2_res <- chisq.test(as.matrix(tab_chi2[, -1]))
chi2_res

cramers_v <- cramerV(as.matrix(tab_chi2[, -1]))
cramers_v

```


The chi-squared test rejects independence (p < 0.001), indicating that commercial categories are statistically associated with gentrification quartiles. However, the effect size is very small (Cramér’s V close to zero), suggesting that this association is economically weak and may largely reflect small differences detectable with large counts.


We fit simple linear models where the dependent variable is the category share within each quartile-year observation. These models provide a complementary view by quantifying average relationships and allowing heterogeneous patterns across categories.

```{r}
#| echo: false
#| message: false
#| warning: false
reg_simple <- lm(part ~ score_quartile, data = df_quartiles)
summary(reg_simple)

```

The simple regression shows no average linear relationship between gentrification intensity and category shares, consistent with the weak effect size obtained above.
...
#| message: false
#| warning: false
reg_inter <- lm(part ~ score_quartile * categorie, data = df_quartiles)
summary(reg_inter)

```


The interaction model tests whether the relationship between gentrification and commercial structure differs by category. Overall, interaction terms provide limited evidence of strong category-specific compositional change across gentrification quartiles. Taken together, these results suggest that gentrification dynamics identified through prices, income, and population composition are not reflected in large short-run shifts across broad BPE business categories.
...
From our representations and tests it is apparent that the share of types of businesses for each quartile of our gentrification score do not differ significantly, nor do they differ significantly across time. 

Although our analysis is successful in identifying neighborhoods that have undergone gentrification it is not strongly reflected in the BPE. 
...
But this could also be explained by our data , the timespan we studied is too short. They're aren't particularly a lot of 
